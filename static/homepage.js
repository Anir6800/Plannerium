...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const initTheme = () => {
        const savedTheme = localStorage.getItem('plannerium-theme') ||
    ...
    const toggleTheme = () => {
        ...
        localStorage.setItem('plannerium-theme', newTheme);
                ...
                demoOutput.innerHTML = `
                    <div class="demo-results">
                        ...
                        <div class="demo-actions">
                            ...
                            <button class="glass-button primary" onclick="exportDemoPDF()">
    ...
    window.exportDemoPDF = async function() {
        const demoOutput = document.getElementById('demo-output');
        const results = demoOutput.querySelector('.demo-results');

        if (!results) {
            alert('Please generate a demo plan first');
            return;
        }

        const projectData = {
            project_name: 'Demo Project',
            project_goal: 'Build a mobile app for task management with team collaboration features.',
            start_date: '2025-11-15',
            deadline: '2026-02-15',
            hours_per_week: 15,
            task_breakdown: [
                { task: 'UI/UX Design', duration: '2 weeks' },
                { task: 'Frontend Development', duration: '4 weeks' },
                { task: 'Backend API', duration: '3 weeks' },
                { task: 'Testing & QA', duration: '2 weeks' }
            ],
            schedule: {
                title: 'Schedule Overview',
                phases: [
                    { phase: 'Phase 1', weeks: 'Weeks 1-3' },
                    { phase: 'Phase 2', weeks: 'Weeks 4-7' },
                    { phase: 'Phase 3', weeks: 'Weeks 8-11' }
                ]
            },
            risk_analysis: {
                title: 'Risk Analysis',
                risks: [
                    { level: 'Low', description: 'Technical complexity' },
                    { level: 'Medium', description: 'Third-party integrations' },
                    { level: 'High', description: 'User adoption rate' }
                ]
            }
        };
        ...
        try {
            ...
            if (response.ok) {
                ...
                a.href = url;
                a.download = data.filename || 'demo-plan.pdf';
